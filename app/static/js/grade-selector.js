(function () {
  ...
  function filterChoices() {
    ...
    choices.forEach(function (choice) {
      var match = !query || choice.dataset.grade.indexOf(query) !== -1;
      choice.hidden = !match;
      if (match) matches += 1;
    });
  ...
  function selectChoice(choice) {
    ...
    search.value = choice.dataset.grade;
  ...
  search.addEventListener("focus", function () { filterChoices(); openList(); });
  ...
  choices.forEach(function (choice) {
    choice.addEventListener("mousedown", function (event) { event.preventDefault(); selectChoice(choice); search.focus(); });
  });
